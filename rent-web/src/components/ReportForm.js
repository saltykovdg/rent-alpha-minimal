...
class ReportForm extends EditComponent {
    ...
    this.props.form.validateFields((error, values) => {
      if (!error) {
        ...
        if (periodStart <= periodEnd) {
          this.props.onOkFormReport(newObject);
        } else {
  ...
  render() {
    ...
    return (
      <Modal
        visible={this.props.formReportVisible}
        title={this.props.title}
        okText={this.props.intl.messages.buttonExecuteReportUPD}
        onOk={(this.onOk)}
...
export default Form.create()(injectIntl(ReportForm));
