...
class DistrictEdit extends EditComponent {
  render() {
    ...
    return (
      <div>
        <Breadcrumb>
          ...
          <Breadcrumb.Item><Link to={AddressPath.DISTRICT_LIST}><FormattedMessage id="districtTitle" /></Link></Breadcrumb.Item>
        ...
        <Spin spinning={this.props.isLoading}>
          <Form horizontal onSubmit={this.handleSubmit}>
            {baseFields}
            <FormItem label={this.props.intl.messages.districtFieldName}>
              {this.getInputField('name', object.name)}
            </FormItem>
            <FormItem>
              <Button type="primary" htmlType="submit"><FormattedMessage id="buttonSave" /></Button>
              <Button className="pull-right" onClick={() => this.forwardTo(AddressPath.DISTRICT_LIST)}>
...
export default Form.create()(injectIntl(DistrictEdit));
