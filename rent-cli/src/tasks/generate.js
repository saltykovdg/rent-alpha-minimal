...
import getRentConfig from './getRentConfig';

class generate {
  constructor(args) {
      this.ui = new UI({});
      this.rentConfig = getRentConfig();
      this.availableBluePrints = this.rentConfig.blueprints.map(bp => bp.name);
  ...
  processArgs = args => {
    ...
    this.blueprint = this.rentConfig.blueprints[this.availableBluePrints.indexOf(args[0])];
